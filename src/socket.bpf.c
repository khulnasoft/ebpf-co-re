...
static __always_inline short unsigned int set_idx_value(khulnasoft_socket_idx_t *nsi, struct inet_sock *is)
{
    // Read Family
...
#if defined(KHULNASOFT_CONFIG_IPV6)
...
static __always_inline int khulnasoft_common_inet_csk_accept(struct sock *sk)
{
    if (!sk)
        return 0;
...
static __always_inline int khulnasoft_common_tcp_retransmit(struct inet_sock *is)
{
...
static __always_inline int khulnasoft_common_tcp_cleanup_rbuf(int copied, struct inet_sock *is, __u64 received)
{
...
static __always_inline int khulnasoft_common_tcp_set_state(struct inet_sock *is, int state)
{
...
static __always_inline int khulnasoft_common_tcp_close(struct inet_sock *is)
{
    khulnasoft_socket_t *val;
    __u16 family;
    khulnasoft_socket_idx_t idx = { };
...
static inline int khulnasoft_common_udp_recvmsg(struct sock *sk)
{
    __u64 pid_tgid = bpf_get_current_pid_tgid();
...
static __always_inline int khulnasoft_common_udp_recvmsg_return(struct inet_sock *is, __u64 received)
{
    __u32 key = KHULNASOFT_CONTROLLER_APPS_ENABLED;
    __u64 pid_tgid = bpf_get_current_pid_tgid();
...
static __always_inline int khulnasoft_common_tcp_connect(struct inet_sock *is, int ret,
                                                      enum socket_counters success,
                                                      enum socket_counters err)
{
    libkhulnasoft_update_global(&tbl_global_sock, success, 1);

    if (ret < 0) {
        libkhulnasoft_update_global(&tbl_global_sock, err, 1);
        return 0;
    }
...
int BPF_KRETPROBE(khulnasoft_inet_csk_accept_kretprobe)
{
    struct sock *sk = (struct sock*)PT_REGS_RC(ctx);

    return khulnasoft_common_inet_csk_accept(sk);
}

SEC("kprobe/tcp_v4_connect")
int BPF_KRETPROBE(khulnasoft_tcp_v4_connect_kprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    return khulnasoft_common_tcp_connect(is, 0, KHULNASOFT_KEY_CALLS_TCP_CONNECT_IPV4,
                                      KHULNASOFT_KEY_ERROR_TCP_CONNECT_IPV4);
}

SEC("kretprobe/tcp_v4_connect")
int BPF_KRETPROBE(khulnasoft_tcp_v4_connect_kretprobe)
{
    int ret = (int)PT_REGS_RC(ctx);

    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    return khulnasoft_common_tcp_connect(is, ret, KHULNASOFT_KEY_CALLS_TCP_CONNECT_IPV4,
                                      KHULNASOFT_KEY_ERROR_TCP_CONNECT_IPV4);
}

SEC("kprobe/tcp_v6_connect")
int BPF_KRETPROBE(khulnasoft_tcp_v6_connect_kprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    return khulnasoft_common_tcp_connect(is, 0, KHULNASOFT_KEY_CALLS_TCP_CONNECT_IPV6,
                                      KHULNASOFT_KEY_ERROR_TCP_CONNECT_IPV6);
}

SEC("kretprobe/tcp_v6_connect")
int BPF_KRETPROBE(khulnasoft_tcp_v6_connect_kretprobe)
{
    int ret = (int)PT_REGS_RC(ctx);

    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    return khulnasoft_common_tcp_connect(is, ret, KHULNASOFT_KEY_CALLS_TCP_CONNECT_IPV6,
                                      KHULNASOFT_KEY_ERROR_TCP_CONNECT_IPV6);
}

SEC("kprobe/tcp_retransmit_skb")
int BPF_KPROBE(khulnasoft_tcp_retransmit_skb_kprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));

    return khulnasoft_common_tcp_retransmit(is);
}

// https://elixir.bootlin.com/linux/v5.6.14/source/net/ipv4/tcp.c#L1528
SEC("kprobe/tcp_cleanup_rbuf")
int BPF_KPROBE(khulnasoft_tcp_cleanup_rbuf_kprobe)
{
    int copied = (int)PT_REGS_PARM2(ctx);
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    __u64 received = (__u64) copied;

    return khulnasoft_common_tcp_cleanup_rbuf(copied, is, received);
}

SEC("kprobe/tcp_set_state")
int BPF_KPROBE(khulnasoft_tcp_set_state_kprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    int state = (int)PT_REGS_PARM2(ctx);

    return khulnasoft_common_tcp_set_state(is, state);
}

SEC("kprobe/tcp_close")
int BPF_KPROBE(khulnasoft_tcp_close_kprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));

    return khulnasoft_common_tcp_close(is);
}

// https://elixir.bootlin.com/linux/v5.6.14/source/net/ipv4/udp.c#L1726
SEC("kprobe/udp_recvmsg")
int BPF_KPROBE(khulnasoft_udp_recvmsg_kprobe)
{
    struct sock *sk = (struct sock*)PT_REGS_PARM1(ctx);

    return khulnasoft_common_udp_recvmsg(sk);
}

SEC("kretprobe/udp_recvmsg")
int BPF_KRETPROBE(khulnasoft_udp_recvmsg_kretprobe)
{
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
    __u64 received = (__u64) PT_REGS_RC(ctx);

    return khulnasoft_common_udp_recvmsg_return(is, received);
}

SEC("kretprobe/tcp_sendmsg")
int BPF_KRETPROBE(khulnasoft_tcp_sendmsg_kretprobe)
{
    int ret = (int)PT_REGS_RC(ctx);
...
int BPF_KPROBE(khulnasoft_tcp_sendmsg_kprobe)
{
...
int BPF_KRETPROBE(khulnasoft_udp_sendmsg_kretprobe)
{
    int ret = (int)PT_REGS_RC(ctx);
    size_t sent = (ret > 0 )?(size_t)ret : 0;
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
...
int BPF_KPROBE(khulnasoft_udp_sendmsg_kprobe)
{
    size_t sent = (size_t)PT_REGS_PARM3(ctx);
    struct inet_sock *is = (struct inet_sock *)((struct sock *)PT_REGS_PARM1(ctx));
...
int BPF_PROG(khulnasoft_inet_csk_accept_fexit, struct sock *sk)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_v4_connect_fentry, struct sock *sk, struct sockaddr *uaddr, int addr_len, int ret)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_v4_connect_fexit, struct sock *sk, struct sockaddr *uaddr, int addr_len, int ret)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_v6_connect_fentry, struct sock *sk, struct sockaddr *uaddr, int addr_len, int ret)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_v6_connect_fexit, struct sock *sk, struct sockaddr *uaddr, int addr_len, int ret)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_retransmit_skb_fentry, struct sock *sk)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_cleanup_rbuf_fentry, struct sock *sk, int copied)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_set_state_fentry, struct sock *sk, int state)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_close_fentry, struct sock *sk)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_udp_recvmsg_fentry, struct sock *sk)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_udp_recvmsg_fexit, struct sock *sk, struct msghdr *msg, size_t len)
{
    if (!sk)
        return 0;
    ...
    return khulnasoft_common_udp_recvmsg_return(is, (__u64)len);
}

SEC("fentry/tcp_sendmsg")
int BPF_PROG(khulnasoft_tcp_sendmsg_fentry, struct sock *sk, struct msghdr *msg, size_t size)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_tcp_sendmsg_fexit, struct sock *sk, struct msghdr *msg, size_t size, int ret)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_udp_sendmsg_fentry, struct sock *sk, struct msghdr *msg, size_t len)
{
    if (!sk)
        return 0;
...
int BPF_PROG(khulnasoft_udp_sendmsg_fexit, struct sock *sk, struct msghdr *msg, size_t len, int ret)
{
    if (!sk)
        return 0;
