...
#ifndef _KHULNASOFT_CORE_COMMON_H_
#define _KHULNASOFT_CORE_COMMON_H_ 1
...
enum KHULNASOFT_EBPF_CORE_IDX {
    KHULNASOFT_EBPF_CORE_IDX_HELP,
    KHULNASOFT_EBPF_CORE_IDX_PROBE,
    KHULNASOFT_EBPF_CORE_IDX_TRACEPOINT,
    KHULNASOFT_EBPF_CORE_IDX_TRAMPOLINE,
    KHULNASOFT_EBPF_CORE_IDX_PID
};

#define KHULNASOFT_EBPF_CORE_MIN_STORE 128

#define KHULNASOFT_EBPF_KERNEL_5_19_0 332544

#define KHULNASOFT_CORE_PROCESS_NUMBER 4096
 ...
 */
static inline void ebpf_core_fill_ctrl(struct bpf_map *map, enum khulnasoft_apps_level map_level)
{
    int fd = bpf_map__fd(map);

    unsigned int i, end = bpf_map__max_entries(map);
    uint64_t values[KHULNASOFT_CONTROLLER_END] = { 1, map_level, 0, 0, 0, 0};
    for (i = 0; i < end; i++) {
 ...
 * invalid.
 */
static inline enum khulnasoft_apps_level ebpf_check_map_level(int value)
{
    if (value < KHULNASOFT_APPS_LEVEL_REAL_PARENT || value > KHULNASOFT_APPS_LEVEL_IGNORE) {
        fprintf(stderr, "\"Error\" : \"Value given (%d) is not valid, resetting to default 0 (Real Parent).\",\n",
                value);
        value = KHULNASOFT_APPS_LEVEL_REAL_PARENT;
   }
 ...
 */
static inline int khulnasoft_libbpf_vfprintf(enum libbpf_print_level level, const char *format, va_list args)
{
...
#endif /* _KHULNASOFT_CORE_COMMON_H_ */
