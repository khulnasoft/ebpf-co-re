...
enum khulnasoft_sync_enum {
    KHULNASOFT_SYNCFS_SYSCALL,
    KHULNASOFT_MSYNC_SYSCALL,
    KHULNASOFT_SYNC_FILE_RANGE_SYSCALL,
    KHULNASOFT_FSYNC_SYSCALL,
    KHULNASOFT_FDATASYNC_SYSCALL,
    KHULNASOFT_SYNC_SYSCALL,

    KHULNASOFT_END_SYNC_ENUM
};
...
static inline void ebpf_disable_local_tracepoints(struct sync_bpf *obj, enum khulnasoft_sync_enum idx)
{
    if (idx != KHULNASOFT_SYNCFS_SYSCALL )
        bpf_program__set_autoload(obj->progs.khulnasoft_syncfs_entry, false);

    if (idx != KHULNASOFT_MSYNC_SYSCALL)
        bpf_program__set_autoload(obj->progs.khulnasoft_msync_entry, false);

    if (idx != KHULNASOFT_SYNC_FILE_RANGE_SYSCALL)
        bpf_program__set_autoload(obj->progs.khulnasoft_sync_file_range_entry, false);

    if (idx != KHULNASOFT_FSYNC_SYSCALL)
        bpf_program__set_autoload(obj->progs.khulnasoft_fsync_entry, false);

    if (idx != KHULNASOFT_FDATASYNC_SYSCALL)
        bpf_program__set_autoload(obj->progs.khulnasoft_fdatasync_entry, false);

    if (idx != KHULNASOFT_SYNC_SYSCALL)
        bpf_program__set_autoload(obj->progs.khulnasoft_sync_entry, false);
}

static inline int ebpf_load_and_attach(struct sync_bpf *obj, int selector, enum khulnasoft_sync_enum idx)
{
    char *name = ebpf_sync_syscall[idx];
    if (!selector) { // trampoline
        bpf_program__set_autoload(obj->progs.khulnasoft_sync_kprobe, false);
        ebpf_disable_local_tracepoints(obj, KHULNASOFT_END_SYNC_ENUM);

        bpf_program__set_attach_target(obj->progs.khulnasoft_sync_fentry, 0,
                                       name);
    } else if (selector == 1) { // kprobe
        ebpf_disable_local_tracepoints(obj, KHULNASOFT_END_SYNC_ENUM);

        bpf_program__set_autoload(obj->progs.khulnasoft_sync_fentry, false);
    } else { // tracepoint
        bpf_program__set_autoload(obj->progs.khulnasoft_sync_kprobe, false);
        bpf_program__set_autoload(obj->progs.khulnasoft_sync_fentry, false);
    ...
    if (selector != 1) // Not kprobe
        ...
        obj->links.khulnasoft_sync_kprobe = bpf_program__attach_kprobe(obj->progs.khulnasoft_sync_kprobe,
                                                                    false, name);
        ret = libbpf_get_error(obj->links.khulnasoft_sync_kprobe);
    }
...
static int ebpf_fcnt_tests(int (*fcnt)(int), enum khulnasoft_sync_enum idx, int selector)
{
...
static int ebpf_msync_tests(int selector)
{
    ...
    int ret = ebpf_load_and_attach(obj, selector, KHULNASOFT_MSYNC_SYSCALL);
    if (!ret) {
        int fd = bpf_map__fd(obj->maps.tbl_sync) ;
        ret = msync_tests(fd);
    } else
...
static int ebpf_sync_file_range_tests(int selector)
{
    ...
    int ret = ebpf_load_and_attach(obj, selector, KHULNASOFT_SYNC_FILE_RANGE_SYSCALL);
    if (!ret) {
        int fd = bpf_map__fd(obj->maps.tbl_sync) ;
...
static int ebpf_test_sync(int selector)
{
    ...
    int ret = ebpf_load_and_attach(obj, selector, KHULNASOFT_SYNC_SYSCALL);
    if (!ret) {
        int fd = bpf_map__fd(obj->maps.tbl_sync) ;
        ret = sync_tests(fd);
    }
...
int main(int argc, char **argv)
{
    ...
    if (!selector) {
        ...
        if (bf)
            selector = ebpf_find_functions(bf, selector, ebpf_sync_syscall, KHULNASOFT_END_SYNC_ENUM);
    }

    ret = ebpf_fcnt_tests(syncfs, KHULNASOFT_SYNCFS_SYSCALL, selector);
    if (!ret)
    ...
    if (!ret)
        ret = ebpf_fcnt_tests(fsync, KHULNASOFT_FSYNC_SYSCALL, selector);

    if (!ret)
        ret = ebpf_fcnt_tests(fdatasync, KHULNASOFT_FDATASYNC_SYSCALL, selector);
